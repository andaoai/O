...
// color = 该柱专属身份色（低饱和暗调，用于环内外边线，避免喧宾夺主压过「此刻高亮」）；
// borderWidth = 主环边线粗细。年/月/时为「主时辰」三环，边线更粗以突出；日/分/秒稍细。
// 月/日/时各自的「天干环 / 地支环」与其六十甲子同属一组，共用此柱色（更细）抱团。
const PILLARS: {
  id: PillarId
  name: string
  thickness: number
...
// border = { color, width }：给环的内外边线上柱色，用以区分六柱身份（缺省走暗灰细边）。
function buildRingData(
  ...
  return {
    ...
    circleColor: border?.color ?? source.circleColor,
    circleWidth: border?.width ?? source.circleWidth,
...
// border：与所属柱六十甲子环同柱色描边（更细），使一组三环视觉抱团。
function buildTianganRing(jiaziIndex: number, border?: { color: string; width: number }) {
  const { stems, kongwang } = xunInfo(jiaziIndex)
  const activeBranch = branchOf(jiaziIndex)
  return {
    ...twelveShichen,
    circleColor: border?.color ?? twelveShichen.circleColor,
    circleWidth: border?.width ?? twelveShichen.circleWidth,
...
// border：与所属柱同柱色描边（更细），与天干环、六十甲子环成组。
function buildBranchRing(jiaziIndex: number, border?: { color: string; width: number }) {
  const activeBranch = branchOf(jiaziIndex)
  return buildRingData(activeBranch, branchColorOf(activeBranch), twelveShichen, false, border)
}

// 一柱拆「天干空亡环 + 十二地支环」两环（外干内支），日柱/时柱共用此模板。
// border = 所属柱身份色，组内两环共用（比六十甲子主环更细一档），整组抱团。
function pillarSubRings(jiaziIndex: number, border: { color: string; width: number }) {
  return [
    {
      component: DataRingComp,
      thickness: 28,
      gapBefore: 0,
      props: { data: buildTianganRing(jiaziIndex, border) }
    },
    {
      component: DataRingComp,
      thickness: 28,
      gapBefore: 0,
      props: { data: buildBranchRing(jiaziIndex, border) }
    }
  ]
}
  ...
  for (const p of PILLARS) {
    out.push({
      ...
      props: {
        ...
        // 环内外边线上该柱专属身份色（年金/月橙/日红/时青/分蓝/秒紫），年月时更粗。
        data: buildRingData(
          currentIndices.value[p.id],
          nayinColorOf(currentIndices.value[p.id]),
          sixtyJiazi,
          p.vertical,
          { color: p.color, width: p.borderWidth }
        ),
    ...
    // 组内子环描同柱色（比主环细一档），使「六十甲子 + 天干 + 地支」三环成组抱团。
    if (p.id === 'month' || p.id === 'day' || p.id === 'hour') {
      out.push(
        ...pillarSubRings(currentIndices.value[p.id], {
          color: p.color,
          width: Math.max(0.8, p.borderWidth - 0.8)
        })
      )
    }
  }
