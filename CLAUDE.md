...
- **飞伏图盘 (feifu)** — Jing Fang's eight-palace 64-hexagram fei-fu (飞伏) relations: fei is manifest, fu is hidden, yin and yang are each other's dwelling. Outer ring is Jing Fang's eight palaces; 64 directed arrows radiate inward from every hexagram to its 8 pure-palace host, forming a radial convergence.
...
│     GuanDouView, FeifuView, SuzhouStellarMapView             │
...
| `liuqin.ts` | 六亲派生 | 纳甲 + 本宫五行 → 生我/我生/克我/我克/比和 → 父母/子孙/官鬼/妻财/兄弟 |
...
| `feifu.ts` | 飞伏关系 | 京房八宫 64 卦飞伏（fei / fu）方向 |
...
└── compass/                       # 九个罗盘页 + 一览页（全部 layout: compass）
...
│   ├── feifu/                       # 飞伏图盘专用
│   │   └── FeifuArrowOverlay.vue   # 64 卦飞伏箭头径向叠加
│   ├── gua/                         # 单卦静态图（古籍笔记专用，非罗盘环组件）
│   │   └── SingleGuaChart.vue      # 六爻爻线 + 纳甲六亲 + 世应静态 SVG
...
└── utils/                          # Layer 5: Utility Layer (Pure Functions)
    ...
    ├── najia.ts                    # 京房纳甲工具
    ├── liuqin.ts                   # 六亲派生：纳甲 + 本宫五行 → 每爻五行/六亲
    ├── wuxing.ts                   # 五行相生相克 + 配色
...
**View 专属工具位** 走另一条路：`<Teleport to="#sidebar-view-tools">` 把 View 自己的控件（如 `SuzhouStellarMapView` 的朝向切换、`FeifuView` 的三条筛选栏）投递到 Sidebar 常驻的挂载点。空 Teleport 目标由 CSS `:has(:empty)` 自动隐藏，无需 JS 观察。
...
除环组件外，`SingleGuaChart` 也在此处全局注册，供古籍笔记（如 `docs/books/京氏易傳/*.md`）直接内嵌单卦静态图：

```md
<SingleGuaChart :value="63" :yao-text="['潜龙勿用', '见龙在田', ...]" />
```

它是**非环组件**、完全静态（无 time / 无 ClientOnly），单纯派生自 `utils/liuqin.ts`。属于「盘面之外的书籍插图」这一类：与罗盘共用配色与数据源，但独立于五层架构，故放在 `src/components/gua/` 而非 `rings/` 或 `centers/`。
