...
| `guan-dou/SolarTermsRing.vue` | Point | 观斗节气 | 太阳黄经→赤道环节气刻度 |
| `TwentyFourMountainsRing.vue` | Segment | 二十四山 | 廿四山方位 + 真北朝向旋转 + 太阳所在山高亮 |
| `GuaRelationTextRing.vue` | Segment | 卦名文字环 | 64 卦名沿环径向排布（飞伏图盘外圈） |
| `qi-men-dun-jia/LiuJiaziDayRing.vue` | Segment | 奇门六轮甲子 | 360 段日刻度（60 甲子 × 6 元） |
| `qi-men-dun-jia/JuShuRing.vue` | Segment | 奇门九局数 | 阳遁 1→9 顺 / 阴遁 9→1 逆，72 段 × 5° |
...
2. **判断归属**:
   - **专用** → 放进 `src/components/rings/<罗盘 kebab-id>/`（子目录名对齐 `docs/compass/<id>.md`）；文件名**不带盘名前缀**
   - **共享**（≥2 个 View 引用，或需被 md 内嵌 → 全局注册） → 放 `src/components/rings/` 根目录
3. **创建组件**: `src/components/rings/[<compass-id>/]XxxRing.vue`，遵循统一范式
4. **提取工具逻辑**: 复杂计算下沉到 `utils/xxx.ts` 纯函数
5. **集成到视图**: 在 RingStack 配置中加入，只传 `time: controlledTime`
...
│   │   ├── GuaRelationTextRing.vue # Domain: 卦名文字环（GuaRelation 外圈 + md 内嵌）
│   │   │  ─── 各罗盘专用（子目录 = 罗盘 kebab-case ID） ───
│   │   ├── jingfang/
│   │   │   └── LiuRiQiFenScaleRing.vue    # 京房：365.25-day fine scale ring
│   │   ├── planet-mansion/
│   │   │   ├── SkyChart.vue               # 天极投影星图（内嵌 HelioOrbits）
│   │   │   ├── SolarTermsSkyRing.vue      # 天星版节气（黄经→赤经映射）
│   │   │   └── SinglePlanetRing.vue       # 单行星深度研究（孤立组件保留）
│   │   ├── tropical-year/
│   │   │   └── MoonPhaseRing.vue          # 当日月相实时绘制
│   │   ├── guan-dou/
│   │   │   ├── MonthEstablishRing.vue     # 斗建（月建）赤道环
│   │   │   ├── MonthGeneralRing.vue       # 月将（大吉/功曹…） 环
│   │   │   ├── HourShichenRing.vue        # 12 时辰实时环
│   │   │   ├── SunDiurnalRing.vue         # 白昼-曙暮-夜三层弧
│   │   │   └── SolarTermsRing.vue         # 观斗节气（黄经→赤道映射）
│   │   ├── gua-relation/
│   │   │   └── DeriveStatRing.vue         # 推衍层收敛统计发光环
│   │   ├── fengshui24/
│   │   │   └── TwentyFourMountainsRing.vue # 二十四山方位环（真北 + 磁偏角校正）
│   │   └── qi-men-dun-jia/
│   │       ├── LiuJiaziDayRing.vue        # 六轮甲子日环（60 × 6 = 360 段）
...
│   │   │  ─── 各罗盘专用（子目录 = 罗盘 kebab-case ID） ───
│   │   ├── planet-mansion/
│   │   │   └── HelioOrbits.vue            # 日心行星轨道盘（被 SkyChart 内嵌）
│   │   ├── tropical-year/
│   │   │   └── LeapInfoCenter.vue         # 闰月 / 月相信息中心
│   │   ├── guan-dou/
│   │   │   └── BeidouCenter.vue           # 北斗七星 + 紫微垣 + 勾陈一 + 地平圈
│   │   ├── suzhou-stellar-map/
│   │   │   ├── SuzhouSkyMap.vue           # 苏州石刻星图圆心（拱极北斗 + 岁差）
│   │   │   └── WorldMapCenter.vue         # 世界地图底层（Natural Earth 110m，LST 锁定）
│   │   └── qi-men-dun-jia/
...
│   ├── viz/                         # 文档可视化组件（古籍笔记、概念文等）
│   │   ├── SingleGuaChart.vue      # 爻位/爻辞/卦五行/卦阴阳/爻线/当位/纳甲/五行/六亲
│   │   ├── PolygonViz.vue          # 多边形数可视化
│   │   ├── DivChartViz.vue         # 除法结构可视化
│   │   ├── TripleViz.vue           # 三元结构可视化
│   │   └── ChongViz.vue            # 重卦结构可视化
...
│   ├── mansionStars.ts             # 二十八宿主星表
│   ├── chineseStarCatalog.ts       # 中华星官目录
│   ├── cities.ts                   # 城市表（定位反查/风水盘用）
...
│       ├── twentyFourMountains.ts  # 二十四山（风水盘）
...
罗盘控制面板由 `CompassLayout` 单点挂载（`<CompassSidebar>` 一份供 10 个 View 共享），不是每个 View 各自持有。这就带来一个 Vue 树的现实问题：
...
它是**非环组件**、完全静态（无 time / 无 ClientOnly），单纯派生自 `utils/guaInfo.ts`。属于「盘面之外的文档可视化」这一类：与罗盘共用配色与数据源，但独立于五层架构，故与其他文档图形统一放在 `src/components/viz/`，而非 `rings/` 或 `centers/`。后续文档可视化组件也统一归入该目录。
...
- `src/components/viz/SingleGuaChart.vue` — **文档可视化范式**：非环、静态、纯派生 utils 的组件写法
